...
import { Sidebar, SidebarHeader, SidebarContent, SidebarFooter } from "@/components/ui/sidebar.tsx";
import { ContextMenu, ContextMenuTrigger, ContextMenuContent, ContextMenuItem } from "@/components/ui/context-menu.tsx";
    ...
    <>
      <ContextMenu>
        <ContextMenuTrigger>
          <Button
            asChild
            variant={"ghost"}
            className={cn(["w-full justify-start px-6 py-2", "text-lg", "font-medium"])}
          >
      ...
      <Dialog open={showDelete} onOpenChange={setShowDelete}>
        <DialogContent className={cn(["flex", "items-center", "justify-center", "p-0"])}>
          <DeleteListDialog listId={item.id} category={item.title} onClose={handleClose} />
        </DialogContent>
      </Dialog>

      <Dialog open={showUpdate} onOpenChange={setShowUpdate}>
        <DialogContent className={cn(["flex", "items-center", "justify-center", "p-0"])}>
          <UpdateListDialog listId={item.id} category={item.title} onClose={handleClose} />
        </DialogContent>
      </Dialog>
    </>
  );
  ...
  return (
    <Sidebar className={cn("w-1/5 flex flex-col justify-between")} collapsible="icon">
      ...
      <SidebarContent className={cn(["overflow-hidden", "flex", "justify-center"])}>
        {authStore.user && (
          <ScrollArea className={cn(["flex", "mx-4", "border", "rounded-md", "h-68", "overflow-y-auto"])}>
            <div className={cn([""])}>
              <div
                className={cn([
                  "text-base",
      ...
      <SidebarFooter className={cn("w-full", "flex", "h-1/3", "justify-end")}>
        ...
        {authStore.user && <AddList onAddSuccess={fetchLists} />}
      </SidebarFooter>
