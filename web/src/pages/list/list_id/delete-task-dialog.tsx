...
function DeleteTaskDialog({ taskId, taskName, onClose, onDelete }: DeleteTaskDialogProps) {
  ...
  return (
    <>
      <div className={cn(["flex", "flex-col", "p-2", "m-1", "min-h-32", "w-full", "gap-10"])}>
        <div className={cn(["flex", "gap-2", "items-center", "text-2xl", "font-bold"])}>
          <TrashIcon className={cn(["size-8"])} />
          删除任务
        </div>
        {/*"opacity-50"*/}
        <p className={cn(["text-lg", "font-medium", "text-center"])}>你确定要删除任务 {taskName} 吗？</p>
        <div className={cn(["flex", "justify-end"])}>
          <Button level={"error"} variant={"tonal"} size={"sm"} onClick={() => handleDelete()}>
            {loading ? "删除中..." : "确定"}
          </Button>
        </div>
      </div>
